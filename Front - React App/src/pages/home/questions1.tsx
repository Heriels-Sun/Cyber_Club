...
import { useContractUtils, useSignlessUtils, useVoucherUtils } from "hooks";
import { MAIN_CONTRACT, SPONSOR_MNEMONIC, SPONSOR_NAME } from "consts";
...
function GeneralQuestion() {
  ...
  const { 
    ...
    currentVoucherId,
    setCurrentVoucherId
  } = useContext(dAppContext); // TODO
  const [signlessModalOpen, setSignlessModalOpen] = useState(false);
  ...
  const { 
    ...
    generateNewVoucher,
    vouchersInContract
  } = useVoucherUtils(SPONSOR_MNEMONIC, SPONSOR_MNEMONIC); 
...
const createVoucherForCurrentPolkadotAccount = async (): Promise<HexString> => {
  ...
  const signer = async (puntos: number) => {
    if (!signlessAccount || !currentVoucherId) {
      setSignlessModalOpen(true);
      return;
    }

    if (!accounts || !account) {
      alert.error('Account not ready');
      return;
    }
